...
def get_configs(db: Session, skip: int = 0, limit: int = 100, config_key: str = None):
    """分页查询配置列表，支持按配置键搜索"""
    ...
    if config_key:
        query = query.filter(
            or_(
                Config.config_key.like(f"%{config_key}%"),
                Config.description.like(f"%{config_key}%")
            )
        )
...
def get_configs_count(db: Session, config_key: str = None):
    """获取配置总数"""
    query = db.query(Config)
